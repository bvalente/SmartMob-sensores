...
def create_client_socket():
    ...
    try:
        ...
        logging.info('client_socket successfully created')
...
def connect_to_server(my_sock, host, port):
    ...
    try:
        ...
        logging.info('Client successfully connected to server- IP address:{}, port {}'.format(host, port))
    except socket.error as msg:
...
def message_from_client(my_sock):
    try:
        ...
        logging.debug('message_from_client: {}'.format(data_message))
    except socket.error as msg:
        logging.exception('Failed to receive message from client: {}'.format(msg))
        my_sock.close()
        sys.exit()
